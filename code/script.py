...
lbl.fit(list(pd.concat((train['manager_id'], test['manager_id']), axis = 0).values))
train['manager_id'] = lbl.transform(list(train['manager_id'].values))
test['manager_id'] = lbl.transform(list(test['manager_id'].values))

temp = pd.concat([train.manager_id,pd.get_dummies(y_train)], axis = 1).groupby('manager_id').mean()
...
try:
    bst.save_model('./models/million_xgb_manager_skill.model')
except:
    print ("ERR: Could not save trained model")
